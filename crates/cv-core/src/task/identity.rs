...
mod tests {
    ...
    fn non_identity_events_ignore_tokens_entirely() {
        ...
        assert_eq!(authorize(&dir, "agent:w", false, Some("wrong")).unwrap(), Decision::Proceed);
    }
    ...
    fn raw_token_never_touches_disk() {
        ...
        assert!(!on_disk.contains("super-secret-token"), "raw token must not be stored: {on_disk}");
        assert!(on_disk.contains(&token_hash("super-secret-token")), "hash is what is stored");
    }
